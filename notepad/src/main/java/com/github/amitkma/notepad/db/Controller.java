package com.github.amitkma.notepad.db;
...
public class Controller {
     ...
     */
    public <T extends DatabaseModel> ArrayList<T> findNotes(Class<T> cls, String[] columns,
            String where, String[] whereParams, int sortId) {
    ...
    public void addCategoryCounter(long categoryId, int amount) {
        ...
        try {
            Cursor c = db.rawQuery(
                    "UPDATE " + OpenHelper.TABLE_NOTES + " SET " + OpenHelper.COLUMN_COUNTER + " = "
                            + OpenHelper.COLUMN_COUNTER + " + ? WHERE " + OpenHelper.COLUMN_ID
                            + " = ?",
                    new String[]{
    ...
    public void deleteNotes(String[] ids, long categoryId) {
        ...
        try {
            ...
            if (categoryId == DatabaseModel.NEW_MODEL_ID) {
                ...
                Cursor c = db.rawQuery(
                        "UPDATE " + OpenHelper.TABLE_NOTES + " SET " + OpenHelper.COLUMN_COUNTER
                                + " = " + OpenHelper.COLUMN_COUNTER + " - ? WHERE "
                                + OpenHelper.COLUMN_ID + " = ?",
                        new String[]{
        ...
        try {
            if (note.id > DatabaseModel.NEW_MODEL_ID) {
                ...
                if (note instanceof Note) {
                    ...
                    Cursor c = db.rawQuery(
                            "UPDATE " + OpenHelper.TABLE_NOTES + " SET " + OpenHelper.COLUMN_COUNTER
                                    + " = " + OpenHelper.COLUMN_COUNTER + " + 1 WHERE "
                                    + OpenHelper.COLUMN_ID + " = ?",
                            new String[]{
