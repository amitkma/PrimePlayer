...
import com.github.amitkma.dictionary.model.Example;
import com.github.amitkma.dictionary.model.MeaningModel;
import com.github.amitkma.dictionary.model.Sense;
...
public class MeaningAdapter extends RecyclerView.Adapter<MeaningAdapter.CustomViewHolder> {
    ...
    public CustomViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new CustomViewHolder(
                LayoutInflater.from(parent.getContext()).inflate(R.layout.item_meaning, parent,
                        false));
    }
    ...
    public void onBindViewHolder(CustomViewHolder holder, int position) {
        if (meaningModelList != null) {
            if (meaningModelList.get(position).getLexicalCategory() != null
                    && !meaningModelList.get(position).getLexicalCategory().isEmpty()) {
                holder.nounVerbCategoryTv.setText(meaningModelList.get(
                        position).getLexicalCategory().trim());
            } else {
                holder.nounVerbCategoryTv.setVisibility(View.GONE);
            }
            if (meaningModelList.get(position).getPhoneticWord() != null && !meaningModelList.get(
                    position).getPhoneticWord().isEmpty()) {
                holder.phoneticWordTv.setText(meaningModelList.get(position).getPhoneticWord());
            } else {
                holder.phoneticWordTv.setVisibility(View.GONE);
            }
            if (meaningModelList.get(position).getAudioFile() != null && !meaningModelList.get(
                    position).getAudioFile().isEmpty()) {
                holder.audioIv.setVisibility(View.VISIBLE);
            } else {
                holder.audioIv.setVisibility(View.GONE);
            }
            if (meaningModelList.get(position).getSenseList() != null && meaningModelList.get(
                    ...
                    if (data.examples != null && data.examples.size() > 0) {
                        for (Example example : data.examples) {
                            if (!example.text.isEmpty()) {
                                exam.append((++number) + ". " + example.text + "\n");
                            }
                        }
                ...
                public void onClick(View v) {
                    ...
                    try {
                        mediaPlayer.setDataSource(
                                meaningModelList.get(getAdapterPosition()).getAudioFile());
